...
router.post('/authenticate', function(req, res, next){
    ...
    User.getUserByEmail(email, function(err, user){
            ...
            User.comparePassword(passport, user.password, function(err, isMatch){
                    if (err) throw err;
                    if (isMatch) {
                        const usr = {"_id": user._id, "name": user.name, "email": user.email, "password": user.password};
                        const token = jwt.sign(usr, config.secret,{expiresIn: 604000});
                        return res.json({
                            success: true,
                            token: 'bearer '+token,
                            user:{
...
router.post('/orderConfirmationEmail', passport.authenticate('jwt', { session: false}), function(req, res, next){
    ...
    emailTemplate.orderConf().render(req.body, function(err, email){
        if(err){
            ...
            var from_email = new helper.Email('cloud7shisha@gmail.com');
            var to_email = new helper.Email(req.body.customer.email);
            var subject = 'Order confirmation: '+req.body.orderNumber;
            var content = new helper.Content('text/html', email.html);
...
function sendOrderConfirmationToUs(order){
    emailTemplate.orderConfToUs().render(order, function(err, email, res){
        if(err){
            ...
            var from_email = new helper.Email('cloud7shisha@gmail.com');
            var to_email = new helper.Email("cloud7shisha@gmail.com");
