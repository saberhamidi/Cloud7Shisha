...
export class OrderService {
  ...
  updateBasket(basket: Object, navigateToBasket?: boolean){
    ...
    return this.http.put('basket/updateBasket/'+this.basket_id, basket, {headers:headers}).map(res =>res.json()).subscribe(data =>{
  ...
  createBasket(items: any, navigateToBasket?: boolean, repeatOrder?: boolean){
    ...
    let headers = new Headers();
    headers.append('Content-Type','application/json');
    return this.http.post('basket/addItem', {headers:headers}).map(res =>res.json()).subscribe(data=>{
        if(data.success){
  ...
  fetchBasket(basket_id){
    return this.http.get('basket/fetchBasket/'+basket_id).map(res => res.json());
  }
  ...
  saveOrder(order: Object, token: string){
    ...
    headers.append('Content-Type','application/json');
    return this.http.post('order', order, {headers:headers}).map(res => res.json());
  }
  ...
  loadOrdersByUserId(userId: string, token){
    ...
    headers.append('Authorization',token);
    headers.append('Content-Type','application/json');
    return this.http.get('order/user/'+userId, {headers:headers}).map(res => res.json());
  }
  ...
  applyDiscount(basket){
    let headers = new Headers();
    headers.append('Content-Type','application/json');
    return this.http.put('basket/updateDiscount/', basket, {headers:headers}).map(res =>res.json());
  }
}